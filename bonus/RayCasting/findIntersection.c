...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/03 22:29:55 by mmakboub          #+#    #+#             */
/*   Updated: 2023/03/03 23:33:43 by mmakboub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	find_horzintersection(t_player *player, t_ray *ray, \
		t_map_info *map, bool gethorzhit)
{
	t_cordinates	step;
	...
	return (getdis(player, ray->horzwallhit.x, ray->horzwallhit.y, gethorzhit));
}
	...
	bool getverthit)
{
	...
	while (ray->vertwallhit.x >= 0 && ray->vertwallhit.x < map->maxlenmap * \
	TILE_SIZE && ray->vertwallhit.y >= 0 && \
