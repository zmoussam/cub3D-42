...
void put_minimap(t_collect_data *data)
{
  ...
  while(i < MINI_MAP_HEIGHT)
  {
    ...
    while(j < MINI_MAP_WIDTH)
    {
      ...
      start_minimap.y < data->map_info->maplines * MINI_MAP_TILE_SIZE) && \
      (start_minimap.x / MINI_MAP_TILE_SIZE) < ft_strlen(data->map_info->map[(int)(start_minimap.y / MINI_MAP_TILE_SIZE)]))
      {
